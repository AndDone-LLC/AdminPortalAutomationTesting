...
export class commonUtils {
  ...
  static writeExcel(fileName: string, data: Record<string, any>[], sheetName = 'Sheet1'): void {
    try {
      const filePath = this.resolvePath(fileName);
  ...
  static writeCSV(fileName: string, data: Record<string, any>[], delimiter = ','): void {
    try {
      if (!data.length) return;
      const filePath = this.resolvePath(fileName);
      const headers = Object.keys(data[0] ?? {}).join(delimiter);
      const rows = data.map(row => Object.values(row).join(delimiter)).join('\n');
  ...
  static async editTextFile(filePath: string, search: string | RegExp, replace = ''): Promise<void> {
    try {
  ...
  static async getFileCount(folderPath: string): Promise<number> {
    try {
      const entries = await fs.promises.readdir(path.resolve(folderPath), { withFileTypes: true });
  ...
  // ================== PDF LINK ==================
  static async clickLinkInsidePdf(page: any, filePath: string, index = 0): Promise<void> {
  ...
  // ================== COMMAND ==================
  static async runCommand(folderPath: string, command: string): Promise<string> {
    try {
      const { stdout } = await execAsync(command, { cwd: path.resolve(folderPath) });
      return stdout;
    } catch (error) {
